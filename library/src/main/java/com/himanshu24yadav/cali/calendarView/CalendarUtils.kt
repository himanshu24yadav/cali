...
fun Calendar.getDatesRange(toCalendar: Calendar): List<Calendar> =
        if (toCalendar.before(this)) {
            getCalendarsBetweenDates(toCalendar.time, this.time,toIncludeLast = true)
        } else {
            getCalendarsBetweenDates(this.time, toCalendar.time,toIncludeLast = false)
        }

fun Calendar?.getMonthAsText() : String {
    return CalendarProperties.monthHeaderList[this?.get(Calendar.MONTH) ?: 0]
}

private fun getCalendarsBetweenDates(dateFrom: Date, dateTo: Date,toIncludeLast:Boolean = false): List<Calendar> {
    val calendars = mutableListOf<Calendar>()

    val calendarFrom = Calendar.getInstance().apply { time = dateFrom }
    val calendarTo = Calendar.getInstance().apply { time = dateTo }

    val daysBetweenDates = TimeUnit.MILLISECONDS.toDays(calendarTo.timeInMillis - calendarFrom.timeInMillis)

    val loopThrough = when {
        toIncludeLast -> 1..(daysBetweenDates)
        else -> 0 until daysBetweenDates
    }
...
fun Calendar.getEnabledDayFrom(calendarProperties: CalendarProperties, daysCount:Int) : Calendar {
    val toCalendar = Calendar.getInstance()
    daysCount.minus(1)
    toCalendar.time = this.time

    toCalendar.add(Calendar.DAY_OF_YEAR,daysCount)
    return toCalendar.apply {
        calendarProperties.maximumDate?.let { maxDate ->
            if(this.after(maxDate)) {
                add(Calendar.DAY_OF_YEAR,-(daysCount*2))
            }
        }
    }
}
